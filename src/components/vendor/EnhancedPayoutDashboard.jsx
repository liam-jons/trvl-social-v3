  ...
  const loadReconciliationSummary = async () => {
    try {
      ...
      const { data, error } = await supabase
        ...
        `)
        .eq('vendor_account_id', vendorStripeAccountId)
        .order('created_at', { ascending: false })
        .limit(1)
        .single();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Payment & Payout Management</h2>
          <p className="text-gray-600">Comprehensive payment tracking, reconciliation, and payout management</p>
        </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className={`text-2xl font-bold ${getHealthScoreColor(reconciliationSummary?.health_score || 0)}`}>
                  {reconciliationSummary?.health_score?.toFixed(1) || '0.0'}%
                </p>
              </div>
            </div>
          </CardContent>
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="flex-1">
                <h3 className="text-lg font-semibold text-yellow-800">
                  Payment Reconciliation Issues Detected
                </h3>
                <p className="text-yellow-700">
                  {reconciliationSummary.discrepancy_count} discrepancies found in recent reconciliation.
                  Health score: {reconciliationSummary.health_score.toFixed(1)}%
                </p>
              </div>
              <Button
        ...
        <div className="flex justify-between items-center">
          <TabsList>
            ...
            <TabsTrigger value="reconciliation">
              Reconciliation
              {reconciliationSummary?.discrepancy_count > 0 && (
                <Badge variant="destructive" className="ml-2">
                  {reconciliationSummary.discrepancy_count}
                </Badge>
              )}
            </TabsTrigger>
                    ...
                    <div key={payout.id} className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-600">{formatDate(payout.created_at)}</p>
                        </div>
                      </div>
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    ...
                    <Badge variant={reconciliationSummary?.health_score >= 95 ? 'default' : 'secondary'}>
                      {reconciliationSummary?.health_score?.toFixed(1) || '0.0'}%
                    </Badge>
                  </div>
                  <div className="flex justify-between items-center">
                    <span>Active Discrepancies:</span>
                    <Badge variant={reconciliationSummary?.discrepancy_count > 0 ? 'destructive' : 'default'}>
                      {reconciliationSummary?.discrepancy_count || 0}
                    </Badge>
                  </div>
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-sm text-gray-600">
                      {reconciliationSummary?.created_at
                        ? formatDate(reconciliationSummary.created_at)
                        : 'Never'
                      }
                    </span>
                  </div>
              ...
              <CardContent>
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold">
                      {reconciliationSummary?.reconciliation_rate?.toFixed(1) || '0.0'}%
                    </span>
                  </div>
                </div>
              </CardContent>
